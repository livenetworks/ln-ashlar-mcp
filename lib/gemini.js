...
export function runGemini(prompt, { config, execFileFn = execFile } = {}) {
	...
	return new Promise((resolve, reject) => {
		const home = cfg.geminiHome || process.env.HOME || "/home/mcp";
		...
		try {
			child = execFileFn(cfg.geminiBin, args, opts, (err, stdout, stderr) => {
				if (err) {
					if (err.code === "ENOENT")
						return done(reject, new GeminiError("CLI_MISSING",
							`${cfg.geminiBin} binary not found in the server's PATH. Check the mcp-server service PATH and installation.`));
					...
					return done(reject, new GeminiError("CLI_ERROR", `${cfg.geminiBin} CLI failed: ${blob.trim().slice(0, 500)}`));
				}
				let parsed;
				...
				if (!text) {
					const detail = stderr ? stderr.trim().slice(0, 300) : 'Gemini JSON had no usable "response" field.';
					return done(reject, new GeminiError("NON_JSON", detail));
				}
				return done(resolve, { text, parsed });
			});
		} catch (e) {
			return done(reject, new GeminiError("CLI_MISSING", `Failed to spawn ${cfg.geminiBin} CLI: ${e.message}`));
		}
