...
export const DEFAULT_CONFIG = {
	...
	maxIterations: 3,
	geminiBin: "agy",
	geminiHome: process.env.HOME || "/home/mcp", // isolated HOME for the runner
		...
		try {
			child = execFileFn(cfg.geminiBin, args, opts, (err, stdout, stderr) => {
				if (err) {
					...
					const cleanMsg = sanitizeCliError(err, stdout, stderr, cfg.geminiBin);
					return done(reject, new GeminiError("CLI_ERROR", `Reviewer CLI failed:\n${cleanMsg}`));
				}
				let parsed;
			...
			return done(reject, new GeminiError("CLI_MISSING", `Failed to spawn Reviewer CLI: ${e.message}`));
		}
