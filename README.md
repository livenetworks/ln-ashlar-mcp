...
MCP tool (`tools/review_plan.js`) that sends a plan (architectural or implementation) to an independent AI reviewer via `claude-cli` or `gemini-cli`. Supports automatic complementary cross-review: requests from Claude agents are automatically routed to Gemini/Antigravity for review, and requests from Antigravity/Gemini agents are routed to Claude for review. You can also explicitly specify `reviewer: "claude" | "gemini" | "auto"`. Authentication uses each CLI's native credentials (Claude subscription / OAuth or `~/.gemini/gemini-credentials.json`) — no secrets exist in this repo. The tool is stateless — the calling agent manages the loop: draft → review → revise, up to 3 iterations; on iterations 2–3, `previous_feedback` is passed along; stops on `APPROVE` or iteration 3. Supported `plan_type` options: `architecture` and `implementation`. Configuration: `config/gemini.json` and `config/claude.json` (model, timeout, concurrency, max iterations, isolated runner `HOME`/`cwd`). Logs: api-key id, engine, plan_type, iteration, chars in/out, duration, verdict, model. Security: CLIs run in isolated text-only mode with empty/isolated `cwd`. Each call is also logged to a dedicated audit log (`logs/review-audit-*.log`). After loop completion (APPROVE or iteration 3), the calling agent can make an optional call with `wrap_up: true` to receive a brief final summary of the entire review process.
